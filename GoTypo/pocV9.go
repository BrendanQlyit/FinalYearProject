...
func runPermutations(targets []string) {
		...
		for _, target := range targets {
			...
			printTldReport("TLD", tldAttack(sanitizedDomain))
...
func doTldLookups(Technique, Domain, out chan<- Record, geolocate bool) {
	defer wg.Done()
	r := new(Record)
	r.Technique = Technique
	r.Domain = Domain
...
func runTldLookups(technique string, results []string, out chan<- Record,  geolocate bool) {
	for _, r := range results {
		wg.Add(1)
		go doTldLookups(technique, r, out, geolocate)
	}
}

func printTldReport(technique string, results []string) {
	out := make(chan Record)
	w.Init(os.Stdout, 18, 8, 0, '\t', 0)

	if option == 4{
		runTldLookups(technique, results, out, true)
	}else if option == 1{
		for _, result := range results {
			...
			for _, result := range results {
				printTldResults(w, technique, result)
			}
	}else if option == 3{
		runTldLookups(technique, results, out, false)
}
...
func printTldResults(writer *tabwriter.Writer, technique, result string) {
